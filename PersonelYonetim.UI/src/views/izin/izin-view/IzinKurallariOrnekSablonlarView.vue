<template>
  <div class="flex-1 transition-all duration-300">
    ...
    <main class="p-6">
        ...
        <div v-for="template in templates" :key="template.id" class="bg-white dark:bg-neutral-800 rounded-lg shadow-sm overflow-hidden">
          <div class="p-6">
            <div class="flex justify-between items-start mb-4">
              <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-200">{{ template.name }}</h3>
              <span class="px-2 py-1 text-xs font-semibold rounded-full" :class="template.paid ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300' : 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300'">
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
              <div>
                <span class="block text-sm font-medium text-gray-500 dark:text-gray-400">Hak Ediş</span>
                <span class="block text-gray-800 dark:text-gray-200">{{ template.entitlement }}</span>
              </div>
              <div>
      ...
      <!-- Şablon Detay Modalı -->
      <div v-if="selectedTemplate" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
        <div class="bg-white dark:bg-neutral-800 rounded-lg shadow-lg max-w-2xl w-full">
          <div class="p-6">
            <div class="flex justify-between items-center mb-4">
              <h3 class="text-xl font-semibold text-gray-800 dark:text-gray-200">{{ selectedTemplate.name }}</h3>
              <button @click="selectedTemplate = null" class="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">
                <i class="fas fa-times"></i>
              </button>
            </div>
            
            <div class="mb-4">
              <span class="inline-block px-2 py-1 text-xs font-semibold rounded-full mb-2" :class="selectedTemplate.paid ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300' : 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300'">
            ...
            <div class="grid grid-cols-2 gap-4 mb-6">
              <div>
                <span class="block text-sm font-medium text-gray-500 dark:text-gray-400">Hak Ediş</span>
                <span class="block text-gray-800 dark:text-gray-200">{{ selectedTemplate.entitlement }}</span>
              </div>
              <div>
                <span class="block text-sm font-medium text-gray-500 dark:text-gray-400">Gün</span>
                <span class="block text-gray-800 dark:text-gray-200">{{ selectedTemplate.days }}</span>
              </div>
            </div>
