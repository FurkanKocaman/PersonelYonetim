...
// Seçilen şirket bilgilerini props olarak alıyoruz
const props = defineProps<{
  sirket: SirketModel;
}>();

// Modal'ı kapatmak ve şirket bilgilerini düzenlemek için emit tanımlıyoruz
defineEmits<{
  (event: "closeModal", closeModal: boolean): void;
  (event: "editSirket", sirket: SirketModel): void;
}>();
</script>
  ...
  >
    <div class="relative p-4 max-w-4xl w-full max-h-full">
      <div class="relative bg-white rounded-lg shadow-sm dark:bg-neutral-800 w-full">
        ...
        >
          <h3 class="text-xl font-semibold text-gray-900 dark:text-white">Şirket Detayları</h3>
          <button
            type="button"
          ...
          <div class="space-y-4 w-full">
              ...
              <div class="flex flex-col mr-2 w-full">
                <div class="mb-4">
                  ...
                  <div class="grid grid-cols-2 gap-4">
                    <div>
                      <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Şirket Adı</p>
                      <p class="text-base text-gray-900 dark:text-white">{{ props.sirket.ad }}</p>
                    </div>
                    <div>
                      <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Durum</p>
                      <p class="text-base text-gray-900 dark:text-white">
                        <span 
                          :class="props.sirket.isActive ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300' : 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300'" 
                          class="px-2 py-1 rounded-full text-xs font-medium"
                        >
                          {{ props.sirket.isActive ? 'Aktif' : 'Pasif' }}
                        </span>
                      </p>
                    </div>
                    <div>
                      <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Oluşturulma Tarihi</p>
                      <p class="text-base text-gray-900 dark:text-white">{{ new Date(props.sirket.createdAt).toLocaleDateString('tr-TR') }}</p>
                    </div>
                    <div>
                      <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Oluşturan</p>
                      <p class="text-base text-gray-900 dark:text-white">{{ props.sirket.createUserName }}</p>
                    </div>
                  </div>
                </div>
                ...
                <div class="mb-4">
                  ...
                  <div class="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p class="text-base text-gray-900 dark:text-white">{{ props.sirket.iletisim.eposta }}</p>
                    </div>
                    <div>
                      <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Telefon</p>
                      <p class="text-base text-gray-900 dark:text-white">{{ props.sirket.iletisim.telefon }}</p>
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div class="mb-4">
                  ...
                  <div class="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p class="text-base text-gray-900 dark:text-white">{{ props.sirket.adres.ulke }}</p>
                    </div>
                    <div>
                      <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Şehir</p>
                      <p class="text-base text-gray-900 dark:text-white">{{ props.sirket.adres.sehir }}</p>
                    </div>
                    <div>
                      <p class="text-sm font-medium text-gray-500 dark:text-gray-400">İlçe</p>
                      <p class="text-base text-gray-900 dark:text-white">{{ props.sirket.adres.ilce }}</p>
                    </div>
                  </div>
                  <div class="mt-2">
                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Tam Adres</p>
                    <p class="text-base text-gray-900 dark:text-white">{{ props.sirket.adres.tamAdres }}</p>
                  </div>
                </div>
                
                <div v-if="props.sirket.aciklama" class="mb-4">
                  <h4 class="text-lg font-medium text-gray-900 dark:text-white mb-2">Açıklama</h4>
                  <p class="text-base text-gray-900 dark:text-white">{{ props.sirket.aciklama }}</p>
                </div>
              </div>
            </div>
            ...
            <div class="flex justify-end space-x-2 pt-4 border-t dark:border-gray-600 border-gray-200">
              <button
                ...
                @click="$emit('editSirket', props.sirket)"
              >
                Düzenle
              </button>
              <button
