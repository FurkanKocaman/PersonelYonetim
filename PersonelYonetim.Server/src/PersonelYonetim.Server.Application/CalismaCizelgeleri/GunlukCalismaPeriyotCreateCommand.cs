    ...
    ) : IRequestHandler<GunlukCalismaPeriyotCreateCommand, Result<string>>
{
    ...
    {
        var personelAtama = await personelGorevlendirmeRepository.Where(p => p.PersonelId == request.PersonelId && p.IsActive & !p.IsDeleted).Include(p => p.CalismaTakvimi).ThenInclude(p => p!.CalismaGunler).FirstOrDefaultAsync();
